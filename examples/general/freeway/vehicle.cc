#pragma ident   "@(#)vehicle.cc	1.10    02/02/05 SMI"
...
Vehicle::recalc (Vehicle *in_front, const int limit, void *neighbors)
{
  ...
  if (restrict_change) restrict_change--;
  ...
  if (in_front)	// There is a car in front of me.
	...
	{				      // car length behind car in front
	  if (velocity > in_front->velocity)
	    state = VSTATE_BRAKE;
	  else              state = VSTATE_MAINTAIN;
	}
      else if (ratio < 0.7) state = VSTATE_BRAKE;
      else if (ratio < 0.9 ||
	       velocity > limit_speed(limit)) 
	state = VSTATE_COAST;
      else if (ratio > 1.1 && 
	       velocity < limit_speed(limit) * 0.98)
  ...
  else				// I am the lead car; nobody ahead
    {
      ...
      if      (ratio > 1.0) state = VSTATE_COAST;
      else if (ratio < 0.9) state = VSTATE_ACCELERATE;
      else                  state = VSTATE_MAINTAIN;
    }
  ...
  switch (state)
    {
      ...
      case VSTATE_ACCELERATE: velocity += 1.00;      break;
...
void
Vehicle::draw (Display *display, Drawable pix, GC gc, int x, int y, 
	       int direction_right, int scale, int xorg, int yorg, 
	       int selected)
{
  this->xloc(x);
  this->yloc(y);
...
//
void
draw_selection(Display *dpy, Drawable pix, GC gc, 
	       int x, int y, int width, int height, int scale)
{
  extern unsigned long color_red, color_white;
  ...
  XSetForeground(dpy, gc, (pixel_depth > 1) ? color_red : color_white);

  XFillRectangle(dpy, pix, gc, l, t, w, scale);             // top
  XFillRectangle(dpy, pix, gc, l, t, scale, h);		    // left
  XFillRectangle(dpy, pix, gc, l, t + h - scale, w, scale); // bottom
  XFillRectangle(dpy, pix, gc, l + w - scale, t, scale, h); // right
}
